...
class CommonTestSetup(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        self._metadata_path = os.path.join(self._pipeline_dir, "tfx_metadata", "metadata.db")
        self._serving_model_dir = os.path.join(self._pipeline_root, "serving_model")

    def setTransformOutputs(self):
        transform_graph_output = os.path.join(self._pipeline_root, "Transform/transform_graph")
        self.transform_output = os.path.join(transform_graph_output, os.listdir(transform_graph_output)[0])
        ...
        transformed_examples_output = os.path.join(self._pipeline_root, "Transform/transformed_examples")
        self.transformed_train_files = os.path.join(transformed_examples_output, 
                                                    os.listdir(transformed_examples_output)[0], 
                                                    "train/*")
        self.transformed_eval_files = os.path.join(transformed_examples_output,
                                                   os.listdir(transformed_examples_output)[0], 
                                                   "eval/*")
